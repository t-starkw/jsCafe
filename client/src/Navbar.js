...
export default function Navbar({ props }) {
  return (
    <div className="fixed top-0 left-0 h-screen w-16 m-0 flex flex-col bg-gray-900 text-white shadow-lg">
      <nav>
        <ul>
          <CustomLink data-tooltip-id="home" data-tooltip-content="Home" to="/">
            <SideBarIcon icon={<BiCoffee size="28" />} />
          </CustomLink>
          <Tooltip id="home" place="right" />
          <CustomLink to="menu" data-tooltip-id="menu" data-tooltip-content="Menu">
            <SideBarIcon icon={<BiFoodMenu size="28" name="menu" />} />
          </CustomLink>
          <Tooltip id="menu" place="right" />
          <CustomLink to="order" data-tooltip-id="order" data-tooltip-content="Order">
            <SideBarIcon icon={<TbPigMoney size="28" />} />
          </CustomLink>
          <Tooltip id="order" place="right" />
          <SideBarIcon data-tooltip-id="dark" data-tooltip-content="DarkMode" icon={<MdDarkMode size="28" />} />
          ...
          <div className="fixed bottom-0 left-0 w-16 m-0 flex flex-col">
            {Auth.loggedIn() ? (
              <>
                ...
                <CustomLink to="profile" data-tooltip-id="profile" data-tooltip-content="Profile">
                  <SideBarIcon icon={<CgProfile size="28" />} />
                </CustomLink>
                <Tooltip id="profile" place="right"/>
                {/* Logout */}
                <CustomLink onClick={Auth.logout} data-tooltip-id="logout" data-tooltip-content="Logout">
                  <SideBarIcon icon={<MdLogout size="28" />} />
                </CustomLink>
                <Tooltip id="logout" place="right"/>
              </>
            ) : (
              <CustomLink to="login" data-tooltip-id="login" data-tooltip-content="Login/Signup">
